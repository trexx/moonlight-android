...
## 25. H.264 `level_idc` patching on the Homatics
...
## 31. Guide LED following the battery

"Battery level" is a fifth value of the Xbox button brightness setting. The guide LED is
monochrome, so following the battery means brightness steps, taken from the presets so each one is
a level the user has already seen: full → bright `0x2F`, medium → normal `0x14`, low → dim `0x0A`,
and critical → a **slow blink** (`LED_BLINK_SLOW`, `0x04`) at normal intensity, because a warning
has to look like one and a dimmer steady LED across a room does not. A pad with no battery fitted
(type 0, running off its cable) goes bright. Before the first report, and for any level the spec
does not define, the pad shows the normal start value. The ladder is `GuideButtonLed.forBattery()`,
on the Java side so it is tested; the native side only applies what it is handed and refuses a
pattern outside `enum LedMode`.

The LED command is now built in one place, `Controller::applyLed()`: `startDevice()` applies
whatever is stored, and `Controller::setLed()` — the new `setLedNative` JNI entry — stores the
battery ladder's command and sends it if the device has started, or leaves it for `startDevice()`
if not, since a command sent in Arrival goes unanswered.

Also fixed on the way: on the cabled path the read thread runs before the Java object exists, and a
status that landed in that window used to arm the driver's change filter with nobody listening, so
Java heard nothing until the battery actually moved — hours, on a full pad. The filter now stays
unarmed until a report is delivered; MS-GIPUSB 2.2.9 has the pad repeat status every second for its
first ten seconds, so the next one is at most a second away.

Nothing here is on a per-frame path: one GIP frame per *level* change, on the driver's read thread,
deduplicated against charge-only changes. Like section 22, the setting is read once in
`UsbDriverService.onCreate()`, so the procedure is "change setting, leave the stream, start it
again".
...
- [ ] **A battery-less cabled pad goes bright.** Type 0 in the status byte, whatever the level
      field says (the spec has such a pad report critical in the level bits). Needs a pad with the
      pack removed, on the wired GIP path (`wiredPadAudio` on).
...
- [ ] **The cabled pad, wired GIP path.** Same ladder through `WiredController`, plus the
      first-status fix above: the LED should reflect the pad within a couple of seconds of it being
      claimed, not only after the battery next moves.
- [ ] **Both ABIs load.** `setLedNative` is a new `(JII)Z` entry in the controller's JNI table; a
      mismatch fails at library load, so the symptom is the adapter or cabled pad not starting at
      all rather than the LED misbehaving.
...
| §31 ladder | A rechargeable adapter pad run down to critical, a play-and-charge cable for the charge-only case, and a cabled pad with its battery pack removed for the type-0 case |
